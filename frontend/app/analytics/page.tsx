...
const Tip = ({active,payload,label}:any) => {
  ...
  return (
    <div style={TOOLTIP_STYLE}>
      <div style={{fontWeight:700,color:'#F0F2FF',marginBottom:4,fontSize:13}}>{label}</div>
...
export default function AnalyticsPage() {
  ...
  const pieData = (data.check_type_totals||[]).slice(0,7).map((c:any)=>({
    name:  CL[c.check_type]||c.check_type,
    value: Math.round(c.overcharge),
  }));
  const barData = (data.check_type_totals||[]).map((c:any)=>({
    name:       CL[c.check_type]||c.check_type,
    Freq:       c.count,
    Overcharge: Math.round(c.overcharge),
  }));
              ...
              <BarChart data={barData} layout="vertical" margin={{left:0,right:40}}>
                <defs>
                  <linearGradient id="orangeGradH" x1="0" y1="0" x2="1" y2="0">
                    ...
                    <stop offset="100%" stopColor="#F78F45" stopOpacity={0.6}/>
                  </linearGradient>
                  <linearGradient id="purpleGradH" x1="0" y1="0" x2="1" y2="0">
                    <stop offset="0%" stopColor="#472F91"/>
                    <stop offset="100%" stopColor="#7B5CC4" stopOpacity={0.6}/>
                  </linearGradient>
                </defs>
          ...
          {pieData.length>0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                ...
                <Tooltip
                  formatter={(v:any,name:any)=>[formatINR(v),name]}
                  contentStyle={TOOLTIP_STYLE}
                  labelStyle={{color:'#F0F2FF',fontWeight:700}}
                  itemStyle={{color:'#F0F2FF',fontSize:13}}
                  cursor={false}
                />
                ...
                return (
                  <tr key={ct.check_type}>
                    <td style={{fontWeight:600,color:'var(--text-primary)',whiteSpace:'nowrap'}}>
                      {CL[ct.check_type]||ct.check_type}
                    </td>
          ...
          <div className="table-scroll-wrapper"><table className="data-table">
            <thead>
              <tr>
                <th>Rank</th>
                <th>Provider</th>
                <th className="right">Audits</th>
                <th className="right">Invoices</th>
                <th className="right">Violations</th>
                <th className="right">Total Overcharge</th>
                <th className="right">Per Invoice</th>
                <th>Risk</th>
              </tr>
            </thead>
            <tbody>
              {providers.map((p:any,i:number)=>{
                ...
                const risk   = perInv>200
                  ? {t:'HIGH', c:'var(--red)',    b:'rgba(239,68,68,0.1)',   bd:'rgba(239,68,68,0.2)'}
                  : perInv>50
                  ? {t:'MED',  c:'var(--orange)', b:'rgba(245,121,33,0.1)',  bd:'rgba(245,121,33,0.2)'}
                  : {t:'LOW',  c:'var(--green)',  b:'rgba(16,185,129,0.1)',  bd:'rgba(16,185,129,0.2)'};
                return (
                  <tr key={p.provider}>
